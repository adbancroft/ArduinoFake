...
namespace ProxiedArduinoFakeTTest
{
    ...
    void test_getFake(void)
    {
        ...
        TEST_ASSERT_EQUAL_PTR(nullptr, subject.getFake<IArduino>(nullptr, &proxy));

        IDummy dummy;
        proxy._dummy = &dummy;
        TEST_ASSERT_EQUAL_PTR(&dummy, subject.getFake<IArduino>(nullptr, &proxy));

        // Following should throw exception
        try
        {
            IArduino arduino;
            TEST_ASSERT_EQUAL_PTR(&dummy, subject.getFake<IArduino>(nullptr, &arduino));
            TEST_FAIL();
        }
